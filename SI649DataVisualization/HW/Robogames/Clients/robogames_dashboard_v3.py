...
def trait_bars():
    ...
    robots = robots.reindex(columns=['id','Astrogation Buffer Length', 'AutoTerrain Tread Count', 'Cranial Uplink Bandwidth', 'InfoCore Size', 'Polarity Sinks', 'Repulsorlift Motor HP', 'Sonoreceptors'])
    robots = pd.melt(robots, id_vars=['id'], value_vars=['Astrogation Buffer Length', 'AutoTerrain Tread Count', 'Cranial Uplink Bandwidth', 'InfoCore Size', 'Polarity Sinks', 'Repulsorlift Motor HP', 'Sonoreceptors'])
...
cur_time = st.empty()
table_01 = st.empty()
option = st.selectbox(label="Select View Mode", options = ['Facet by ID','Facet by Trait'])
...
for i in np.arange(0,101):
	...
	df_merged = df_merged.reindex(columns=['id', 'name', 'expires', 'time', 'value', 'Arakyd Vocabulator Model', 'Axial Piston Model', 'Nanochip Model'])
	table_01.write(df_merged.sort_values('expires'))
	
	
	if 'Facet by ID' in option:
