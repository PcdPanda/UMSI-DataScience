...
# Visualization 1
base = (
    alt.Chart(movies_test)
    .transform_filter("datum.year>=1990 & datum.year <= 1997")
    .transform_joinaggregate(
        mean_budget="mean(budget):Q", max_budget="max(budget):Q", groupby=["year"]
    )
    .transform_calculate(
...
dot = (
    line.transform_filter(alt.FieldOneOfPredicate(field="high_budget", oneOf=["High"]))
    .mark_point(filled=True)
    .encode(y=alt.Y("budget:Q"))
)
text = (
    dot.mark_text(align="center", dy=-10)
...
text = base.mark_text().encode(
    x=alt.X("rating:Q", bin=True), y=alt.Y("test_result:N"), text=alt.Text("count():Q")
)
high_light = (
    base.transform_joinaggregate(max_rating="max(rating)", groupby=["test_result"])
    .transform_filter("datum.max_rating>=9")
    .mark_rect(color="red", filled=False)
    .encode(x=alt.X("rating:Q", bin=True), y=alt.Y("test_result:N"))
)
...
dom = (
    ...
    .encode(y=alt.Y("test_result:N"))
    .mark_bar(color="blue")
    .encode(x=alt.X("mean(dom_gross)"), color=alt.Color("mean(dom_gross)"))
)
text_dom = (
    alt.Chart(movies_test)
    .transform_filter(
    ...
    .transform_filter("datum.gross_rank==10")
    .mark_text(align="left", x=10)
    .encode(y=alt.Y("test_result:N"), text=alt.Text("title:N"))
)
dom += text_dom
...
text_int = (
    ...
    .transform_filter("datum.int_rank==10")
    .mark_text(align="left", x=10, color="white")
    .encode(y=alt.Y("test_result:N"), text=alt.Text("title:N"))
)
inter += text_int
...
Vis4 = (
    ...
    .mark_bar()
    .encode(x=alt.X("mean(dollars):Q"), y=alt.Y("finance:N"), color=alt.Y("finance:N"))
